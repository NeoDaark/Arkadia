...
if(isset($_POST['submit'])){ // comprobamos que se ha enviado el formulario
    ...
    if($_FILES['foto']['name'] != ""){ // El campo foto contiene una imagen...
        ...
        if ((($_FILES["foto"]["type"] == "image/gif")
            ...
            resizeImagen($directorio.'/', $foto, 320, 650,$resFoto,$extension);
            unlink($directorio.'/'.$foto);
            
        } else { // El archivo no es JPG/GIF/PNG
            $malformato = $_FILES["foto"]["type"];
            header("Location: imgcrop.php?error=noFormato&formato=$malformato");
            exit;
          }
        
    } else { // El campo foto NO contiene una imagen
        header("Location: imgcrop.php?error=noImagen");
        exit;
    }
